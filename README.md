...
번역된 Sunny Sunday 일정은 `state.json`에 저장됩니다. 각 일정의 시작 시각에는 공지 채널에 그 주 혜택을 자동으로 보내고, 24시간 뒤 자동 메시지를 삭제합니다. `/썬데이` 명령어는 외부 API를 다시 호출하지 않고 저장된 일정 중 가장 가까운 한 주의 항목만 보여줍니다.

Discord에서 `/썬데이`를 입력하면 현재 진행 중인 일정 또는 가장 가까운 다음 Sunny Sunday 한 건을 확인할 수 있습니다.
